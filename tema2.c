...
void build_finger_table() {
    ...
    for (int i = 1; i <= 8; i *= 2){
        int poz = (start + i) % 16;
        self.finger[index].start = poz;
        self.finger[index].node = find_successor_simple(poz);
        index++;
    }   
}
...
int closest_preceding_finger(int key) {
     ...
     ******************************************************/
     for (int i = 3; i >= 0; i--){
        if (!key)
            key = 16;
        if (in_interval(self.finger[i].node, self.id, key - 1))
...
int main(int argc, char **argv) {
     ...
     ************************************************************/
     for (int i = 0; i < nr_lookups; i++){
        LookupMsg *msg = calloc(1, sizeof(LookupMsg));
    ...
    int **buffer = calloc(16, sizeof(int *)); 
    for (int i = 0; i <= 15; i++){
        buffer[i] = calloc(MAX_PATH, sizeof(int));
    }
    int *sizes = calloc(16, sizeof(int));
    if (!nr_lookups){
        LookupMsg *msg = calloc(1, sizeof(LookupMsg));
        ...
        if (status.MPI_TAG == TAG_LOOKUP_REQ)
            ...
            if (!sizes[msg->key]){
                for (int i = 0; i < msg->path_len; i++){
                    buffer[msg->key][sizes[msg->key]] = msg->path[i];
                    sizes[msg->key]++;
                }
            }
            nr++;
            ...
            left--;
        if (!left && nr == nr_lookups)
            break;
        free(msg);
    } 
    for (int i = 0; i < nr_lookups; i++){
        printf("Lookup %d: ", lookups[i]);
        for (int j = 0; j < sizes[lookups[i]] - 1; j++){
            printf("%d -> ", buffer[lookups[i]][j]);
        }
        printf("%d\n", buffer[lookups[i]][sizes[lookups[i]]-1]);
    }
    for (int i = 0; i <= 15; i++){
        free(buffer[i]);
    }
